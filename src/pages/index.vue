<template>
  <v-container class="pa-4 pa-md-6" fluid>
    ...
    <v-alert
      v-if="updateSummaryItems.length"
      class="mb-4"
      color="warning"
      icon="mdi-update"
      type="warning"
    ...
    >
      <div class="d-flex flex-column flex-md-row justify-space-between align-start align-md-center ga-3">
        <div>
          <div class="text-subtitle-1 font-weight-bold">Updates available</div>
...
import { getSystemStatus, getUpdates } from '@/services/api'
...
const updateSummaryItems = computed(() => {
  const components = updatesData.value?.components || []
  return components.filter(component => component.update_available)
})

const updateHeadline = computed(() => {
  if (!updateSummaryItems.value.length) return 'System updates are available.'
  const firstComponent = updateSummaryItems.value[0]
  if (updateSummaryItems.value.length === 1) {
    return `${firstComponent.app} ${firstComponent.kind}: ${firstComponent.current_version} -> ${firstComponent.latest_version}`
  }
  return `${updateSummaryItems.value.length} components have updates available.`
})
