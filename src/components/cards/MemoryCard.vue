  ...
  <glass-card hoverable>
    <v-card-text>
      <div class="d-flex align-center justify-space-between mb-4">
        <div>
          <div class="text-secondary text-caption mb-1">Memory Usage</div>
      ...
      <div class="d-flex justify-space-between align-center mb-3">
        <div class="text-body-2 text-primary-medium">
          <span class="font-weight-bold">
            {{ data?.used_mb ? (data.used_mb / 1024).toFixed(1) : '--' }}
          </span>
          <span class="text-secondary"> GB used</span>
        </div>
        <div class="text-caption text-secondary">
          of {{ data?.total_mb ? (data.total_mb / 1024).toFixed(1) : '--' }} GB
        </div>
      </div>
      ...
      <div class="d-flex justify-space-between text-caption">
        ...
        <div :class="`text-${getStatusVariant(data?.percent)}`">
          {{ data?.available_mb ? (data.available_mb / 1024).toFixed(1) : '--' }} GB
        </div>
      </div>
    </v-card-text>
  </glass-card>
